...
export default function RightSidebar({ title, links, basePath }) {
  ...
  return (
    <div className="w-full p-4 text-sm font-inter sticky top-0 h-screen overflow-y-auto">
      <h2 className="text-lg font-semibold mb-4 term-style-red border-b border-gray-700 pb-2 w-full">
        {title}
      </h2>
        ...
        {links.map((link, i) => (
          <li key={i}>
            <a
              ...
              className="cursor-pointer term-style-blue"
            >
              {link.text}
            </a>
          </li>
